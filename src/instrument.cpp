...
DashboardInstrument::DashboardInstrument(
    wxWindow* pparent, wxWindowID id, wxString title, int cap_flag)
    : wxControl(pparent, id, wxDefaultPosition, wxSize(800, 500), wxBORDER_NONE)
{
    m_title = title;
    m_cap_flag = cap_flag;
...
int DashboardInstrument::GetCapacity() { return m_cap_flag; }
...
void DashboardInstrument::OnPaint(wxPaintEvent& WXUNUSED(event))
{
    ...
    if (!pdc.IsOk()) {
        wxLogMessage(_T("DashboardInstrument::OnPaint() fatal: ")
                     _T("wxBufferedPaintDC.IsOk() false."));
        return;
    }
    ...
    if (!m_drawSoloInPane) {
        ...
        //  Workaround by using plain old DC for title box if text size is too
        //  small
#ifdef __WXMSW__
...
DashboardInstrument_Single::DashboardInstrument_Single(wxWindow* pparent,
    wxWindowID id, wxString title, int cap_flag, wxString format)
...
void DashboardInstrument_Single::SetData(int st, double data, wxString unit)
{
    if (m_cap_flag & st) {
        if (!wxIsNaN(data)) {
            if (unit == _T("C"))
                m_data
                    = wxString::Format(m_format, data) + DEGREE_SIGN + _T("C");
            else if (unit == _T("\u00B0"))
                ...
                m_data
                    = _T(">") + wxString::Format(m_format, data) + DEGREE_SIGN;
            else if (unit == _T("\u00B0R"))
                m_data
                    = wxString::Format(m_format, data) + DEGREE_SIGN + _T("<");
            ...
            /* maybe in the future ...
             ...
             m/s"); else if (unit == _T("K")) // km/h m_data =
             wxString::Format(m_format, data)+_T(" km/h");
             ... to be completed
             */
            else
...
DashboardInstrument_Position::DashboardInstrument_Position(wxWindow* pparent,
    wxWindowID id, wxString title, int cap_flag1, int cap_flag2)
