    ...
    if (node->Type() == TiXmlNode::TINYXML_DOCUMENT) {
        ...
        if (GetDocument())
            GetDocument()->SetError(
                TIXML_ERROR_DOCUMENT_TOP_ONLY, 0, 0, TIXML_ENCODING_UNKNOWN);
        return 0;
    }
...
TiXmlNode* TiXmlNode::InsertBeforeChild(
...
const TiXmlNode* TiXmlNode::IterateChildren(
...
#ifdef TIXML_USE_STL
const std::string* TiXmlElement::Attribute(
...
TiXmlDeclaration::TiXmlDeclaration(const std::string& _version,
    const std::string& _encoding, const std::string& _standalone)
    : TiXmlNode(TiXmlNode::TINYXML_DECLARATION)
{
    version = _version;
    encoding = _encoding;
