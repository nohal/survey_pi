...
int TCMgr::yearoftimet(time_t t) { return ((gmtime(&t))->tm_year) + 1900; }
...
void TCMgr::LoadMRU(void)
{
    ...
    if (mru_file.Open()) {
        ...
        while (!mru_file.Eof()) {
            ...
            psd->station_name
                = (char*)malloc(strlen(str.mb_str(wxConvUTF8)) + 1);
...
void TCMgr::SaveMRU(void)
{
    ...
    if (pmru_head) {
        ...
        while (pmru) {
            ...
            for (int i = 0; i < num_csts; i++) {
                str_sbuf.Printf(
                    _T("%8.4f  %8.4f"), psd->amplitude[i], psd->epoch[i]);
                mru_file.AddLine(str_sbuf);
            }
    ...
    float& tcvalue_prev, bool& w_t)
{
    ...
    w_t = tcvalue_now
        > tcvalue_prev; // w_t = true --> flood , w_t = false --> ebb
...
bool TCMgr::GetTideOrCurrent(time_t t, int idx, float& tcvalue, float& dir)
{
    ...
    */
    ...
    /*
          ...
          else if (strlen(IDX_tzname) > 0) {// Does this entry have a timezone
    assigned? change_time_zone (IDX_tzname); // Use user station timezone utc =
    tadjust = 0;
          }
          else if (meridian % 60) {
             utc=1;
...
Station_Data* TCMgr::find_or_load_harm_data(IDX_entry* pIDX)
{
    ...
    while (pmru) {
        ...
        //                 psd_station_name:      The Narrows, Midchannel, New
        //                 York Harbor, New York Current
    ...
    //      if(1/*!pIDX->IDX_tried_once*/)
    {
        ...
        //    If reference station was recently sought, and not found, don't
        //    bother
        //            if(!strcmp(pIDX->IDX_reference_name,
        //            plast_reference_not_found->mb_str()))
        if (plast_reference_not_found->IsSameAs(
        ...
        if (!psd)
            plast_reference_not_found->Append(
                wxString(pIDX->IDX_reference_name, wxConvUTF8));
...
void TCMgr::fudge_constituents(Station_Data* psd, IDX_entry* pIDX)
{
    ...
    */
    ...
    /*
      if (psd->have_BOGUS)
      {
            pIDX->IDX_ht_off *= pIDX->IDX_ht_off;         // Square offset in
    kts to adjust for kts^2 pIDX->IDX_lt_off *= pIDX->IDX_lt_off;

    //     htleveloff *= htleveloff1; // Square offset in kts to adjust for
    kts^2
  ...
  */
    ...
    */
    ...
    if (pIDX->IDX_ht_time_off || pIDX->IDX_ht_time_off
        || pIDX->IDX_ht_off != 0.0 || pIDX->IDX_lt_off != 0.0
    ...
    if (amplitude == 0.0) {
        ...
        /*
              ...
              if (fabs (fakedatum) >= 100.0 || fakeamplitude >= 100.0)
                fprintf (stderr, "Tidelib warning:  tidal range out of normal
           bounds\n");
        */
    }
}
...
void TCMgr::figure_multipliers()
{
    ...
    figure_amplitude();
    for (a = 0; a < num_csts; a++)
        work[a] = pmsd->amplitude[a] * cst_nodes[a][year - first_year]
            / amplitude; // BOGUS_amplitude?
    //  if (hincmagic)
    //      pick_hinc ();
}
...
time_t TCMgr::tm2gmt(struct tm* ht)
{
    ...
    */
    ...
    /*
      ...
      ht->tm_mon+1, ht->tm_mday, ht->tm_hour, ht->tm_min, ht->tm_sec); barf
      (BADTIMESTAMP);
      }
    */
    return guess;
...
/* The following array de-allocates were added (per original win95
 * implementation) */
void TCMgr::free_cst()
{
    free(cst_speeds);
...
void TCMgr::UserStationFuncs(int func, char* custom_name)
{
    ...
    // Remove - find user station entry and unlink it from the index.
        ...
        case USF_UPDATE :
          ...
          if (pIDX==NULL && pIDX_prev!=NULL && NULL!=(pIDX = (IDX_entry
    *)malloc(sizeof(IDX_entry)))) { pIDX_prev->IDX_next = pIDX;          // New
    entry so link it into the list pIDX->IDX_rec_num   =
    pIDX_prev->IDX_rec_num+1; pIDX->IDX_next      = NULL; pIDX->IDX_tzname    =
    NULL;
          }
          if (pIDX != NULL) {
            ...
            pIDX->IDX_lon         = Ilon;
            pIDX->IDX_lat         = Ilat;
            pIDX->IDX_time_zone   = IDX_time_zone; // this is reference station
    meridian pIDX->IDX_ht_time_off = Ihttimeoff/60; pIDX->IDX_ht_mpy      =
    Ihlevelmult; pIDX->IDX_ht_off      = Ihtleveloff; pIDX->IDX_lt_time_off =
    Ilttimeoff/60; pIDX->IDX_lt_mpy      = Illevelmult; pIDX->IDX_lt_off      =
    Iltleveloff; pIDX->IDX_sta_num     = IDX_sta_num; pIDX->IDX_ref_file_num=
    IDX_ref_file_num; if (pIDX->IDX_tzname != NULL) free(pIDX->IDX_tzname); if
    (strlen(tadjust_last) > 1 && isalpha(tadjust_last[0]) &&
...
// #error Added extra \r to format strings... check it, or convert to wxTextFile
    ...
    pIDX->IDX_tzname = NULL;
    if (7
        != sscanf(index_line, "%c%s%lf%lf%d:%d%*c%[^\r\n]", &pIDX->IDX_type,
    ...
    if (strchr(
            "tcUu", index_line[0])) { // Substation so get second line of info
        IndexFileIO(IFF_READ, 0);

        /*
        cNAA:XX:XX: -122.5500  47.7167 -8:0 Agate Passage, north end
        ^-88   0.80 0 -18   0.70 0   1301 230 32 1 ADMIRALTY INLET (off Bush
        Point), WASH.
        */
        ...
        /*
        cNAP:US:CA: -123.3667 48.3167 -08:00 045 235 Trial Island, 5.2 miles SSW
        of & 0:39 *0.7  0:55 *0.5 0 0 Admiralty Inlet
        */

        else {
            if (9
                != sscanf(index_line, "%*c%d %f %f %d %f %f %d %d%*c%[^\r\n]",
                    &pIDX->IDX_ht_time_off, &pIDX->IDX_ht_mpy,
                    &pIDX->IDX_ht_off, &pIDX->IDX_lt_time_off,
                    &pIDX->IDX_lt_mpy, &pIDX->IDX_lt_off, &pIDX->IDX_sta_num,
                    &pIDX->IDX_ref_file_num, pIDX->IDX_reference_name)) {
                // Had an error so try alternate with timezone name before ref
                // file number
                if (10
                    != sscanf(index_line,
        ...
        if (pIDX->IDX_ref_file_num
            <= 0) { // Find harmonic reference file number
            pIDX->IDX_ref_file_num = 0;
            // Find reference station in index, if no index, it had better be in
            // the first one
            pIDXh = pIDX_first;
            while (pIDXh != NULL
                && strcmp(pIDXh->IDX_reference_name, pIDX->IDX_reference_name))
        ...
        while (IndexFileIO(IFF_READ, 0)) {
            if ((index_line[0] == '#') || (index_line[0] <= ' '))
                ...
                if (num_abv == 0) {
                    ...
                    return (
                        FALSE); // missing at least some data so no valid index
                }
                if (doing_xref++
                    == 0) { // First pass through, flag for second pass
                    IndexFileIO(IFF_SEEK,
                        xref_start); // Position back to start of index
                    // Allocate memory for the array
                    if (NULL
                        == (abbreviation_list = (abbreviation_entry**)malloc(
                                (num_abv + 1) * sizeof(abbreviation_entry*)))) {
                        IndexFileIO(IFF_CLOSE, 0);
                        //                  no_mem_msg();
                        return (FALSE);
                    } else {
                        for (i = 0; i <= num_abv; i++)
                            if (NULL
                                == (abbreviation_list[i]
                                    = (abbreviation_entry*)malloc(sizeof(
                                        abbreviation_entry)))) { // If we can't
                                                                 // allocate..
                                free_abbreviation_list();
                                IndexFileIO(IFF_CLOSE, 0);
                                //                        no_mem_msg();
                        ...
                        abbreviation_list[num_abv]->type
                            = 0; // Flag final entry
                ...
                clean_string(s3);
                if ((!strncmp(s1, "REGION", 6)) || (!strncmp(s1, "COUNTRY", 7))
                    || (!strncmp(s1, "STATE", 5))) {
                    if (doing_xref) {
                        if (allocate_copy_string(
                                &abbreviation_list[num_abv]->short_s, s2)
                            || (allocate_copy_string(
                                &abbreviation_list[num_abv]->long_s, s3))) {
                            free_abbreviation_list();
                            IndexFileIO(IFF_CLOSE, 0);
                            //                     no_mem_msg();
                            return (FALSE);
                        ...
                        if (!strncmp(s1, "REGION", 6))
                            abbreviation_list[num_abv]->type = REGION;
                        else if (!strncmp(s1, "COUNTRY", 7))
                            abbreviation_list[num_abv]->type = COUNTRY;
                        else
                            abbreviation_list[num_abv]->type = STATE;
                    }
                    num_abv++;
                }
                ...
                if (load_index) {
                    if (NULL
                        != (pIDX = (IDX_entry*)malloc(sizeof(IDX_entry)))) {
                        index_in_memory = TRUE;
                        pIDX->IDX_next = NULL;
                        pIDX->IDX_rec_num = num_IDX;
                        pIDX->IDX_tried_once
                            = 0; // master station search control
                        pIDX->Valid15 = 0;
                        ...
                        // We couldn't allocate memory somewhere along the line,
                        // so free all we have so far.
                        //                  no_mem_msg();
                        free_station_index(); // Free any we have allocated so
                                              // far
                    }
                }
                ...
                pHarmonic
                    = (harmonic_file_entry*)malloc(sizeof(harmonic_file_entry));
                if (NULL == pHarmonic) {
                    //               no_mem_msg();
                    free_harmonic_file_list();
                } else {
    ...
    if (rec_num >= 0) {
        if (index_in_memory) {
            if (rec_num == 0)
                ...
                rewound = FALSE;
                pIDXs = pIDX_first;
                while ((--rec_num > 0) && (pIDXs != NULL))
                    pIDXs = (IDX_entry*)pIDXs->IDX_next;
                return (pIDXs);
            }
        } else {
            IDX_rec_num = 0;
            ...
            while (looking_end && IndexFileIO(IFF_READ, 0)) {
                if (!strncmp(index_line, "*END*",
                        5)) // Stop looking when *END* found
                    looking_end = FALSE;
            }
            if (!looking_end) {
        ...
        if (index_in_memory) {
            ...
            if (pIDXs != NULL)
                return (pIDXs = (IDX_entry*)pIDXs->IDX_next);
        } else {
            while (IndexFileIO(IFF_READ, 0)) {
                if (strchr("TtCcIUu", index_line[0])) {
                    IDX_rec_num++;
...
double TCMgr::time2tide(time_t t) { return time2dt_tide(t, 0); }
...
//      int marklev = 0;
        ...
        if (0)
            if ((p > marklev && q <= marklev)
                ...
                if (p < q)
                    ...
                    flags |= 4;
                if (!(flags & 3)) {
                    /* If we're incredibly unlucky, we could miss a tide event
                       if we don't check for it here:
        ...
        if (flags) {
            *tm -= 60;
            /* Don't back up over a transition event, but do back up to where
               the tide changed if possible.  If they happen at the same time,
               then we're off by a minute on the tide, but if we back it up it
...
double TCMgr::_time2dt_tide(time_t t, int deriv)
{
    ...
    for (a = 0; a < num_csts; a++) {
        term = work[a]
            * cos(tempd + cst_speeds[a] * ((long)(t - epoch) + pmsd->meridian)
                + cst_epochs[a][year - first_year] - pmsd->epoch[a]);
        for (b = deriv; b > 0; b--)
            term *= cst_speeds[a];
        dt_tide += term;
...
double TCMgr::time2dt_tide(time_t t, int deriv)
{
    ...
    if (t - this_epoch <= TIDE_BLEND_TIME && this_year > first_year)
        ...
        return blend_tide(
            t, deriv, this_year, -(double)(next_epoch - t) / TIDE_BLEND_TIME);
...
int TCMgr::GetStationIDXbyName(
    wxString prefix, double xlat, double xlon, TCMgr* ptcmgr)
{
    IDX_entry* lpIDX;
    int jx = 0;
    wxString locn;
    ...
    for (int j = 1; j < ptcmgr->Get_max_IDX() + 1; j++) {
        ...
        if (((type == 't') || (type == 'T')) // only Tides
            ...
            // double brg, dist;
            // DistanceBearingMercator(xlat, xlon, lpIDX->IDX_lat,
            // lpIDX->IDX_lon, &brg, &dist); if (dist < distx){ distx = dist;
