...
SurveyOverlayFactory::SurveyOverlayFactory(SurveyDlg &dlg)
	: m_dlg(dlg), m_sdgs(dlg.mysoundings), m_settings(dlg.mySettings)
...
bool SurveyOverlayFactory::RenderGLSurveyOverlay( wxGLContext *pcontext, PlugIn_ViewPort *vp )
{
...
void SurveyOverlayFactory::DrawMessageWindow( wxString msg, int x, int y , wxFont *mfont)
{
    if(msg.empty())
        return;
...
wxColour SurveyOverlayFactory::GetDepthColour(double my_depth){
	...
	if ((my_depth >= 10) && (my_depth < 12)){ return c_spring; }
	if ((my_depth >= 12) && (my_depth < 14)){ return c_cyan;}
	if ((my_depth >= 14) && (my_depth < 20)){ return c_sky; }
	if ((my_depth >= 20) ){ return c_blue;}
...
wxImage &SurveyOverlayFactory::DrawGLText( double value, int precision ){
    ...
    mdc.DrawText(labels, label_offset + xd, yd+4);
...
void SurveyOverlayFactory::DrawGLLine( double x1, double y1, double x2, double y2, double width, wxColour myColour )
{
    {
...
void SurveyOverlayFactory::DrawOLBitmap( const wxBitmap &bitmap, wxCoord x, wxCoord y, bool usemask )
{
    wxBitmap bmp;
    if( x < 0 || y < 0 ) {
        int dx = ( x < 0 ? -x : 0 );
...
void SurveyOverlayFactory::DrawAllSoundingsInViewPort(PlugIn_ViewPort *BBox, bool bRebuildSelList,
        bool bforce_redraw_currents, bool bdraw_mono_for_mask, wxDateTime myTime)
{
	...
	for (std::vector<soundingdata>::iterator it = myfactorysoundings.begin(); it != myfactorysoundings.end(); it++){
...
void SurveyOverlayFactory::DrawGLSoundingMark(int x, int y,  double scale, double depth, int sounding_id, int survey_id, wxColour c)
{
