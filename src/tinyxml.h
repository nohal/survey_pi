...
/**
   ...
   TiXmlVisitor class to handle callbacks. For nodes that contain other nodes
...
protected:
    ...
    */
    ...
    inline static const char* GetChar(
...
public:
    ...
    */
    ...
    /// 'value'
    const TiXmlNode* IterateChildren(
...
#ifdef TIXML_USE_STL
    const TiXmlNode* IterateChildren(
        const std::string& _value, const TiXmlNode* previous) const
    {
        return IterateChildren(_value.c_str(), previous);
    } ///< STL std::string form.
    TiXmlNode* IterateChildren(
        const std::string& _value, const TiXmlNode* previous)
    {
        return IterateChildren(_value.c_str(), previous);
    } ///< STL std::string form.
#endif
    ...
    */
    TiXmlNode* InsertBeforeChild(
        TiXmlNode* beforeThis, const TiXmlNode& addThis);
...
public:
    ...
    */
    ...
    /** QueryBoolAttribute examines the attribute - see QueryIntAttribute().
       ...
       'false' and 'no' are considered false.
    */
    ...
    */
    ...
    int QueryValueAttribute(
        const std::string& name, std::string* outValue) const
    {
        const TiXmlAttribute* node = attributeSet.Find(name);
        if (!node)
            return TIXML_NO_ATTRIBUTE;
    ...
    */
    ...
    /** Load a file using the given FILE*. Returns true if successful. Note that
       this method doesn't stream - the entire object pointed at by the FILE*
            will be interpreted as an XML file. TinyXML doesn't stream in XML
       from the current file location. Streaming may be added in the future.
    */
...
/**
   ...
   this is an incredibly useful thing. Note that TiXmlHandle is not part of the
   TinyXml DOM structure. It is a separate utility class.
   ...
   "Child" ); if ( child )
                        {
                                TiXmlElement* child2 =
   child->NextSiblingElement( "Child" ); if ( child2 )
                                {
   ...
   ).FirstChild( "Element" ).Child( "Child", i ).ToElement(); if ( !child )
                        break;
                // do something
                ++i;
        }
   ...
   Child method is a linear walk to find the element, so this code would iterate
   much more than it needs to. Instead, prefer:
...
/** Print to memory functionality. The TiXmlPrinter is useful when you need to:
   ...
   mode. Before calling Accept() you can call methods to control the printing of
   the XML document. After TiXmlNode::Accept() is called, the printed document
...
public:
    ...
    /** Set the indent characters for printing. By default 4 spaces
            but tab (\t) is also useful, or null/empty string for no
       indentation.
    */
