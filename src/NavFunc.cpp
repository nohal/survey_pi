...
double toDeg(double radians) { return radians * 180 / M_PI; }

double sqr(double sqr2) { return sqr2 * sqr2; }
...
/*
Another potential implementation problem is that the arguments of asin and/or
...
result follows the sign of the divisor or the dividend. (We want the sign to
follow the divisor or be Euclidean. C's fmod and Java's % do not work.) In this
document, Mod(y,x) is the remainder on dividing y by x and always lies in the
range 0 <=mod <x. For instance: mod(2.3,2.)=0.3 and mod(-2.3,2.)=1.7
*/
...
double NMtorad(double distance_NM) { return (M_PI / (180 * 60)) * distance_NM; }

// convert metres to NM
double mtoNM(double metres) { return metres / 1852; }

// convert NM to metres
double NMtom(double NM) { return NM * 1852; }
...
////////////////////////////
/**
 * Calculates rhumb line distance between two points specified by
 * latitude/longitude http://williams.best.vwh.net/avform.htm#Rhumb
 ...
 * As stated in the introduction, North latitudes and West longitudes are
 * treated as positive, and South latitudes and East longitudes negative. It's
 * easier to go with the flow, but if you prefer another convention you can
 * change the signs in the formulae.
 */
...
/**
 * To find the lat/lon of a point on true course brng, distance dist from
 (lat1,lon1)
    ...
    double* lon2)
{
    ...
    if (std::abs(lat) > M_PI / 2)
        return false; //"d too large. You can't go this far along this rhumb
                      // line!"
    double q;
    ...
    double* lat2, double* lon2)
{
    bool dbg = false;
    if (dbg)
        std::cout << "dL lat1 " << lat1 << " lon1: " << lon1 << "brng " << brng
                  << " dist " << dist << std::endl;
    // double ecc = 0.08181919084255; //The eccentricity ecc of the WGS84
    // ellipsoid is
    ...
    if (tolatdegree < -90) {
        if (dbg)
            std::cout << "You are on the South Pole. tolatdegree" << tolatdegree
                      << std::endl;
        tolatdegree = -90;
    }
    ...
    if ((tolatdegree != 90) && (tolatdegree != -90)) {
        double meridional1 = log(tan(M_PI / 4 + fromlat / 2))
            - ecc2 * sin(fromlat) - (ecc4 / 3) * pow(sin(fromlat), 3)
            - (ecc6 / 5) * pow(sin(fromlat), 5)
            - (ecc8 / 7) * pow(sin(fromlat), 7);
        double meridionalmin1 = 10800 * meridional1 / M_PI;
        double meridional2 = log(tan(M_PI / 4 + tolat / 2)) - ecc2 * sin(tolat)
            - (ecc4 / 3) * pow(sin(tolat), 3) - (ecc6 / 5) * pow(sin(tolat), 5)
            - (ecc8 / 7) * pow(sin(tolat), 7);
